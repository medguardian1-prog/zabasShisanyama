...
| `food-07.jpg` | Braai board — beef ribs, chops, chicken wings, pap, three salads, bread (client, 2026-09-03) | square | Signature strip lead, gallery, Platter for 1, and the desktop hero source. |
...
|---|---|---|
| `01-platter-wide.webp` | `food-07.jpg` | Homepage strip, gallery, Platter for 1, **and the hero** |
| `04-platter-wors.webp` | `food-02.jpg` | Homepage strip, Platter for 2, About "It comes out on a board" |
| `02-chops-phuthu.webp` | `plate-beef.jpg` | Homepage strip, gallery, Phuthu & Beef, **and hero-mobile** |
...
- **`hero-mobile.jpg` now comes from the portrait shot**, not from `food-07`.
  Cropping a landscape source to 2:3 would have meant a 1.9x upscale of a narrow
  slice. It also gets a 12% top crop: the phone scrim runs `to top`, so the top
  of the frame is the one part left fully transparent, and the source opens on a
  dark wooden plank that read as a black band above the headline.
- **The mobile hero scrim was strengthened** (mid-band 0.55 -> 0.78, 0.15 -> 0.38).
  The old stops were tuned against a photo with mean brightness ~80; the
  replacement measures ~154, which left the gold eyebrow sitting on lit tomato
  salsa. Desktop scrims are unchanged.
- **`enhance-images.mjs` no longer owns `food-02`, `food-07` or the hero.** The
  stale Upscayl sources are still in `image-src/upscaled/`, so without a guard a
  re-run there would have silently reverted this work. Hero generation moved out
  of that script entirely; `hero-mobile.jpg` had never been generated by it at
  all, which is why it had drifted out of sync with `hero.jpg`.
