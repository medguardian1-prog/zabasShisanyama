...
//
// food-02, food-05, food-06 and food-07 are also skipped: as of 2026-09-03 all
// four are owned by scripts/import-client-photos-2026-09.mjs, which builds them
// from the second batch of client photography in image-src/client-new/. The
// stale Upscayl sources are still sitting in image-src/upscaled/, so without
// this guard a re-run here would silently revert the homepage strip, gallery
// and about page to the old low-resolution shots the client rejected.
//
 ...
 * committed as-is.
 */
// DISABLED 2026-09-03 -- this block was actively corrupting the hero.
//
// It resizes by width only, so it inherited the source's aspect and wrote a
// 1700x1707 near-square. The committed hero.jpg is 2000x1230 landscape, so
// this had not reproduced the live banner for some time; anyone running this
// script silently replaced the wide banner with a square crop.
//
// hero.jpg and hero-mobile.jpg are now committed assets, edited by hand and
// regenerated by nothing. If you reinstate generation here, pass BOTH width
// and height with fit:"cover" and check the output against 2000x1230 first.
